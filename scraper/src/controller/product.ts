...
export const saveProduct = async (
  ...
  try {
    ...
    const coopProducts = await Promise.all(
      listOfCoopPromotionproducts.map(async (product) => {
        const {
          name = product.name,
          price = product.price,
          supermarket = "coop",
    ...
    const conadProducts = await Promise.all(
    ...
    const savedCoopProducts = coopProducts.filter(
