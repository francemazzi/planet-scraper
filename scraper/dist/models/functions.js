...
export const main_eu = async () => {
    ...
    try {
        ...
        const classNameData = ".colHeader.header-overflow.table-header-container";
        const classOfSpan = ".table-header-text";
        const classOfDiv = ".ag-pinned-left-cols-container";
        await page.waitForSelector(classNameData);
        const table_data = await page.evaluate((classOfDiv, classOfSpan) => {
            const container = document.querySelector(classOfDiv);
            const geo_title = Array.from(container.querySelectorAll(`span${classOfSpan}`));
            const data = geo_title.map((elem) => elem.textContent.trim());
            return data;
        }, classOfDiv, classOfSpan);
...
export const conad_promotions = async () => {
    ...
    try {
        ...
        const [loadMorePromotionProductButton] = await page.$x("//div[contains(., 'Carica altri') and contains(@class, 'rt072-disaggregated-block__loadMore')]");
        ...
        const elements = await divTab.$$eval(".rt213-card-product-flyer", (elements) => {
